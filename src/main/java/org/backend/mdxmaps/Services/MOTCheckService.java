...
public class MOTCheckService {
    ...
    public MOTCheckService(RoutingObjects start, RoutingObjects destination, String mot) {
        this.startBuilding = start.getBuilding();
        this.destBuilding = destination.getBuilding();
        this.startLevel = start.getActualLevel();
        this.destLevel = destination.getActualLevel();
        this.mot = mot;
    }
        ...
        switch (mot) {
            case "null":
                ...
                if (startBuilding.equals(destBuilding)) {
                    response.setEntity(new SBSLFactoryService(startBuilding, mot, startLevel));
                } else {
                ...
                if (!startBuilding.equals(destBuilding)) { //Different Buildings
                    if (new RoutingObjects().getBuildingObject(destBuilding).isBuildingWheelChairAccessible() &&
                        ...
                        if (!startCheck && !destCheck) {
                            ...
                            response.setErrorMessage("There are no elevators in " + startBuilding + " and " + destBuilding);
                        } else if (startCheck && !destCheck) {
                            response.setStatus(Status.ERROR);
                            response.setErrorMessage("There are no elevators in " + destBuilding + " to get to " + destBuilding);
                        } else if (!startCheck) {
                            response.setStatus(Status.ERROR);
                            response.setErrorMessage("There are no elevators in " + startBuilding);
                        } else {
                        ...
                        if (!new RoutingObjects().getBuildingObject(destBuilding).isBuildingWheelChairAccessible() &&
                                !new RoutingObjects().getBuildingObject(startBuilding).isBuildingWheelChairAccessible()) {
                            response.setStatus(Status.ERROR);
                            response.setErrorMessage("Both " + startBuilding + " & " + destBuilding + " are not wheelchair accessible");
                        } else if (!new RoutingObjects().getBuildingObject(startBuilding).isBuildingWheelChairAccessible()) {
                            response.setStatus(Status.ERROR);
                            response.setErrorMessage(startBuilding + " is not wheelchair accessible");
                        } else {
                            response.setStatus(Status.ERROR);
                            response.setErrorMessage(destBuilding + " is not wheelchair accessible");
                        }
                    }
                } else { //Same Building.
            ...
            case "stairs":
                if (startBuilding.equals(destBuilding)) { //Same Building
                    if (new RoutingObjects().getBuildingObject(startBuilding).hasStairs) {
                        ...
                        response.setInfoMessage("No stairs in " + startBuilding + ". App switched to elevators for this building");
                        mot = "elevators";
                    }
                    response.setEntity(new SBDLFactoryService(startBuilding, mot, startLevel, destLevel));
                } else { //Diff buildings
                    if (startLevel != 0) {
                        if (new RoutingObjects().getBuildingObject(startBuilding).hasStairs) {
                            ...
                            startEDMethod = "elevators";
                            response.setStatus(Status.INFO);
                            response.setInfoMessage("No stairs in " + startBuilding + ". App switched to elevators for " + startBuilding);
                        }
                    } else {
                        startED = false;
                        startEDMethod = "null";
                    ...
                    if (destLevel != 0) {
                        if (new RoutingObjects().getBuildingObject(destBuilding).hasStairs) {
                            ...
                            destEDMethod = "elevators";
                            response.setStatus(Status.INFO);
                            if (response.getInfoMessage() == null) {
                                response.setInfoMessage("No stairs in " + destBuilding + ". App switched to elevators for " + destBuilding);
                            } else {
                                response.setInfoMessage("No stairs in both buildings, app switched to elevators in " + startBuilding +
                                        " & " + destBuilding);
                            }
                        }
                    } else {
            ...
            case "elevators":
                if (startBuilding.equals(destBuilding)) { //Same Building
                    if (new RoutingObjects().getBuildingObject(startBuilding).hasElevators) {
                        ...
                        mot = "stairs";
                        response.setStatus(Status.INFO);
                        response.setInfoMessage("No elevators in " + startBuilding + ". App switched to stairs for this building");
                    }
                    response.setEntity(new SBDLFactoryService(startBuilding, mot, startLevel, destLevel));
                } else { //Diff Buildings
                    if (startLevel != 0) {
                        if (new RoutingObjects().getBuildingObject(startBuilding).hasElevators) {
                            ...
                            startEDMethod = "stairs";
                            response.setStatus(Status.INFO);
                            response.setInfoMessage("No elevators in " + startBuilding + ". App switched to stairs for " + startBuilding);
                        }
                    } else {
                        startED = false;
                        startEDMethod = "null";
                    ...
                    if (destLevel != 0) {
                        if (new RoutingObjects().getBuildingObject(destBuilding).hasElevators) {
                            ...
                            response.setStatus(Status.INFO);
                            if (response.getInfoMessage() == null) {
                                response.setInfoMessage("No elevators in " + destBuilding + ". App switched to stairs for " + destBuilding);
                            } else {
                                response.setInfoMessage("No elevators in both buildings, app switched to stairs for " + startBuilding +
                                        " & " + destBuilding);
                            }
                        }
                    } else {
